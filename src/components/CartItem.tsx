...
const CartItem: React.FC<CartItemProps> = ({
  ...
  const onClickPlus = () => {
    dispatch(addItem({ id, type, size } as TypeCartItem));
  };

  const onClickMinus = () => {
    dispatch(minusItem({ id, type, size } as TypeCartItem));
  };

  const onClickRemove = () => {
    if (window.confirm(`Удалить товар (${title}, ${type}, ${size} см.)?`)) {
      dispatch(removeItem({ id, type, size } as TypeCartItem));
    }
  };
